...
export interface PlayerState {
  ...
  /**
   * Забег: выигранные, но ещё не потраченные улучшения карт.
   *
   * Появляются в одном случае: пари взято тем же ударом, который закончил бой.
   * Спрашивать «какую карту улучшить» уже не у кого — партии нет, — но и
   * отнимать выигранное нельзя, поэтому счётчик доезжает до забега, и тот
   * спрашивает между боями.
   */
  upgradesOwed: number
}
