...
import {
  ENGINE_VERSION, actorOf, createGame, enumerateLegalActions, redact, redactEvent, reduce,
  type Action, type GameEvent, type GameState, type PlayerId,
} from '@sr/engine'
...
export function createMatch(): { match: Match; seat: PlayerId; token: string } {
  const matchId = id()
  let code = roomCode()
  ...
  const match: Match = {
    ...
    seats: { p1: { token: sign(matchId, 'p1'), connected: 0 }, p2: { token: sign(matchId, 'p2'), connected: 0 } },
    commands: [],
    seenCmdIds: new Map(),
    createdAt: Date.now(),
    joined: ['p1'],
  ...
  return { match, seat: 'p1', token: match.seats.p1.token }
}
...
export function joinByCode(code: string): { match: Match; seat: PlayerId; token: string } {
  ...
  if (match.joined.includes('p2')) {
    throw new MatchError({ code: 'FULL', message: 'That match already has two players.' })
  }
  match.joined.push('p2')
  // A room code is a join ticket, never an auth token: it is single use.
  byCode.delete(code.toUpperCase())
  return { match, seat: 'p2', token: match.seats.p2.token }
}
...
export function updateFor(match: Match, seat: PlayerId, events: readonly GameEvent[]): {
  ...
  return {
    ...
    opponentConnected: match.seats[other].connected > 0,
  }
}
...
export function reap(maxIdleMs = 6 * 60 * 60 * 1000): number {
  ...
  for (const [key, m] of matches) {
    const live = m.seats.p1.connected + m.seats.p2.connected
