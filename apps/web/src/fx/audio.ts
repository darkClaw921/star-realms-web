...
export const SFX = {
  ...
  /**
   * «Мостик»: короткий двойной сигнал. Свойство базы, включённое кнопкой.
   *
   * Отличается от розыгрыша нарочно и заметно: розыгрыш — то, что кладут на
   * стол, активация — то, что с уже лежащего снимают, и путать их на слух
   * значит терять счёт своим базам.
   */
  activate(): void {
    tone({ type: 'triangle', f: N(9), dur: 0.1, gain: 0.13 })
    tone({ type: 'sine', f: N(16), dur: 0.18, gain: 0.1, at: 0.07 })
  },
  ...
  /**
   * «Мостик»: тревожный колокол — удар и нисходящий полутон.
   *
   * Событие никто не разыгрывал: оно вскрылось само и уже применилось к обоим
   * игрокам. Поэтому и звук не похож ни на один «мой» — ни на розыгрыш, ни на
   * покупку: игрок должен поднять глаза, а не отметить очередное своё действие.
   */
  event(): void {
    tone({ type: 'sine', f: N(-12), dur: 0.7, gain: 0.3 })
    tone({ type: 'triangle', f: N(3), dur: 0.5, gain: 0.14, at: 0.02 })
    tone({ type: 'triangle', f: N(2), dur: 0.6, gain: 0.12, at: 0.18 })
    hiss({ type: 'bandpass', f: 2200, dur: 0.12, gain: 0.1, q: 4 })
  },
