...
export const UI = {
  ...
  // забег
  modeRun: 'Забег',
  modeRunDesc: 'Восемь боёв подряд одной колодой: купленное остаётся, ' +
    'авторитет не восстанавливается, поражение заканчивает забег.',
  toRun: 'К забегу',
  runFightWon: 'Бой выигран',
  runFightLost: 'Забег окончен',
  ...
  // Вопрос занимает весь экран, а ответ на него часто зависит от стола: что
  // стоит у соперника, что осталось в ряду, сколько боя набралось. Свёрнутый
  // вопрос никуда не девается и ждёт в углу.
  // Чья карта в списке целей. «Ваша» — не имя места, а предупреждение: под
  // этим свойством может оказаться собственная база.
  ownerMine: 'ваша',
  foldPrompt: 'Свернуть',
  foldPromptHint: 'Посмотреть стол, не отвечая',
  unfoldPrompt: 'Вернуться к выбору',
